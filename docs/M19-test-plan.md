...
Walkthrough for [`M19-plan.md`](M19-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (SetCouncilTally, `/set respect`, Backpack, crate scale). Later work is listed at the bottom of [`M19-plan.md`](M19-plan.md).

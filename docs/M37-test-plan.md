...
Walkthrough for [`M37-plan.md`](M37-plan.md). Automated tests prove the slice; the `sim-cli` / viewer / browser steps below are what you **read** (extra invention kinds, page `/set` `/give`). Later work is at the bottom of [`M37-plan.md`](M37-plan.md).

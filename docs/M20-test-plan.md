...
Walkthrough for [`M20-plan.md`](M20-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (relationship leave/end, stacked worn packs, attach loopback). Later work is at the bottom of [`M20-plan.md`](M20-plan.md).

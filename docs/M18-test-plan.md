...
Walkthrough for [`M18-plan.md`](M18-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (council tally, SetCouncil, `/scrub` catch-up). Later work is listed at the bottom of [`M18-plan.md`](M18-plan.md).

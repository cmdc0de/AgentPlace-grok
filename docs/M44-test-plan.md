...
Walkthrough for [`M44-plan.md`](M44-plan.md). Automated tests prove the slice; the `sim-cli` steps below are what you **read** (WIS board range, CHA Support/PairBond, STR pocket weight). Next slice: later work at the bottom of [`M44-plan.md`](M44-plan.md).

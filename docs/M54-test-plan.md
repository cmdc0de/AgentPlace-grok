...
Walkthrough for [`M54-plan.md`](M54-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (Pickup, auto-cabin, spear range, torch/axe/jar/bread). Next slice: later work at the bottom of [`M54-plan.md`](M54-plan.md).

...
Walkthrough for [`M21-plan.md`](M21-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (pack fill scale, `--connect` log tail, remote `/scrub`). Next slice: [`M52-plan.md`](M52-plan.md).

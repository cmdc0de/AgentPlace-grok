...
Walkthrough for [`M24-plan.md`](M24-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (wire `/set`, `--connect` `/inject`, lockstep `AckTick`). Next slice: [`M61-plan.md`](M61-plan.md).

...
Walkthrough for [`M61-plan.md`](M61-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (dawn wear, out-dir disk walk, bench/shawl/cobble/cake). Next slice: later work at the bottom of [`M61-plan.md`](M61-plan.md).

...
| **M52** | **Planned — next** | [`M52-plan.md`](M52-plan.md) | Day/night clock (default on), world-size CLI, OTLP export |
| After M52 | Not started | listed at the bottom of `M52-plan.md` | protobuf/TLS |
...
| 57 | `M52-plan.md` | Milestone 52 (planned): day/night clock, world-size CLI, OTLP export |
...
Upload or attach the specification files **and** the milestone plans (`M2-plan.md` through `M52-plan.md`) plus this `00-INDEX-AND-HANDOFF.md`. Start the conversation with:

> “Here are the design specifications and milestone plans. Read `00-INDEX-AND-HANDOFF.md` first (progress table). M52 is next (`M52-plan.md`). Later work is at the bottom of that file. Do not expand into items it defers (protobuf/TLS, skeletal animation, Bevy in the browser).”
...
M1–M51 are done. Next is M52 ([`M52-plan.md`](M52-plan.md)).

```
Read docs/00-INDEX-AND-HANDOFF.md, then docs/M52-plan.md.
Implement M52 (day/night clock default on, --width/--height, OTLP/JSON export;

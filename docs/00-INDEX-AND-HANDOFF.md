...
> “Here are the design specifications and milestone plans. Read `00-INDEX-AND-HANDOFF.md` first (progress table). M20 is implemented; later work is listed at the bottom of `M20-plan.md`. Do not expand into items it defers (protobuf/TLS, wire Give).”
...
M1–M20 are done (tag `M20`). Later work is listed at the bottom of [`M20-plan.md`](M20-plan.md).

```
Read docs/00-INDEX-AND-HANDOFF.md, then docs/M20-plan.md.
M20 is implemented (revert relationship_delta on leave; stacked worn packs;
attach safety net). Later work is at the bottom of that file. Do not add

...
> “Here are the design specifications and milestone plans. Read `00-INDEX-AND-HANDOFF.md` first (progress table). M32 is implemented; later work is listed at the bottom of `M32-plan.md`. Do not expand into items it defers (protobuf/TLS, browser).”
...
M1–M32 are done (tag `M32`). Later work is listed at the bottom of [`M32-plan.md`](M32-plan.md).

```
Read docs/00-INDEX-AND-HANDOFF.md, then docs/M32-plan.md.
M32 is implemented (kin_of incentives; household; aging;

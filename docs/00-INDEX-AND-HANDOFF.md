...
| **M24** | Planned — next | [`M24-plan.md`](M24-plan.md) | Wire /set, connect /inject, lockstep ack |
...
| 29 | `M24-plan.md` | Milestone 24 (planned): wire /set, connect /inject, lockstep ack |
...
> “Here are the design specifications and milestone plans. Read `00-INDEX-AND-HANDOFF.md` first (progress table). M24 is planned, not implemented; do not expand into items it defers (protobuf/TLS).”
...
M1–M23 are done (tag `M23`). **Next:** [`M24-plan.md`](M24-plan.md) (planned, not implemented). Later work is listed at the bottom of that file.

```
Read docs/00-INDEX-AND-HANDOFF.md, then docs/M24-plan.md.
M24 is planned (wire /set; sim-cli --connect /inject; lockstep AckTick;
PROTOCOL_VERSION=5). Do not implement until asked. Do not add protobuf
or TLS. CI stays provider=mock with no network.
```

...
Walkthrough for [`M28-plan.md`](M28-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (health/incapacitation, combat viewer FX, force_reflect). Later work is at the bottom of [`M28-plan.md`](M28-plan.md).

...
Walkthrough for [`M52-plan.md`](M52-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (day/night clock, `--width`/`--height`, OTLP/JSON). Next slice: [`M54-plan.md`](M54-plan.md).

...
Walkthrough for [`M39-plan.md`](M39-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (object TOML visuals + LOD, hashed catalog Craft). Next slice: later work at the bottom of [`M39-plan.md`](M39-plan.md).

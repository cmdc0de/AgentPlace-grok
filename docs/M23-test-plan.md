...
Walkthrough for [`M23-plan.md`](M23-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (every-tick Snapshots, LLM barrier, `sim-cli --connect` Control). Next slice: [`M27-plan.md`](M27-plan.md).

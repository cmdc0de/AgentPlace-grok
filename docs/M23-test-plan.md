...
Walkthrough for [`M23-plan.md`](M23-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (every-tick Snapshots, LLM barrier, `sim-cli --connect` Control). Next slice: [`M57-plan.md`](M57-plan.md).

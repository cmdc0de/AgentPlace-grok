...
Walkthrough for [`M53-plan.md`](M53-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (Place tent/cabin/house, night Hunt/Farm, CON dawn). Next slice: later work at the bottom of [`M53-plan.md`](M53-plan.md).

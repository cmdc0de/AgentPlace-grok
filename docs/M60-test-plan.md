...
Walkthrough for [`M60-plan.md`](M60-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (Transfer wear, CPU percent, stool/sash/brick/biscuit). Next slice: later work at the bottom of [`M60-plan.md`](M60-plan.md).

...
Walkthrough for [`M38-plan.md`](M38-plan.md). Automated tests prove the slice; the `sim-cli` / viewer / browser steps below are what you **read** (authored models with primitive fallback, page `/inject` `/scrub`, Ubuntu wasm32). Next slice: [`M44-plan.md`](M44-plan.md).

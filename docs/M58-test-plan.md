...
Walkthrough for [`M58-plan.md`](M58-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (tool durability, millstone station, viewer-frame OTLP, flour/cart/bellows/table). Next slice: [`M59-plan.md`](M59-plan.md).

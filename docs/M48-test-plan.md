...
Walkthrough for [`M48-plan.md`](M48-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (INT invent quality, agent meshes, hot-reload glb). Next slice: later work at the bottom of [`M48-plan.md`](M48-plan.md).

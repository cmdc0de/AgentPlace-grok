...
Walkthrough for [`M41-plan.md`](M41-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (species `[sim]` from objects TOML, DEX melee miss, STR haul caps). Next slice: [`M62-plan.md`](M62-plan.md).

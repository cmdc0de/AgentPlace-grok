...
Walkthrough for [`M51-plan.md`](M51-plan.md). Automated tests prove the slice; the `sim-cli` / viewer / page steps below are what you **read** (sqlite `/metrics`, attack_bonus, extra recipes). Next slice: later work at the bottom of [`M51-plan.md`](M51-plan.md).

...
Walkthrough for [`M22-plan.md`](M22-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (wire Give, remote `/ckpt`, remote `/events`). Next slice: [`M52-plan.md`](M52-plan.md).

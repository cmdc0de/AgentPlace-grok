...
Walkthrough for [`M22-plan.md`](M22-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (wire Give, remote `/ckpt`, remote `/events`). Next slice: [`M57-plan.md`](M57-plan.md).

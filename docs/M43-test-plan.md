...
Walkthrough for [`M43-plan.md`](M43-plan.md). Automated tests prove the slice; the `sim-cli` steps below are what you **read** (WIS toxin detect, CHA speech range/weight, DEX flee). Next slice: later work at the bottom of [`M43-plan.md`](M43-plan.md).

...
Walkthrough for [`M57-plan.md`](M57-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (hammer stone-gather, process CPU/disk, barrel/cloak/pot/lantern). Next slice: later work at the bottom of [`M57-plan.md`](M57-plan.md).

...
Walkthrough for [`M26-plan.md`](M26-plan.md). Automated tests prove the slice; the `sim-cli` / viewer steps below are what you **read** (Reflect/Plan every-N-ticks, record/replay of reflection text). Later work is at the bottom of [`M26-plan.md`](M26-plan.md).

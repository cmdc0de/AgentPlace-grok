...
Walkthrough for [`M46-plan.md`](M46-plan.md). Automated tests prove the slice; the `sim-cli` steps below are what you **read** (CON illness chance, INT plan length, STR gather/hunt, extra recipes, telemetry). Next slice: [`M54-plan.md`](M54-plan.md).

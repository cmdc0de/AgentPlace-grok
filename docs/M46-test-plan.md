...
Walkthrough for [`M46-plan.md`](M46-plan.md). Automated tests prove the slice; the `sim-cli` steps below are what you **read** (CON illness chance, INT plan length, STR gather/hunt, extra recipes, telemetry). Next slice: [`M48-plan.md`](M48-plan.md).
